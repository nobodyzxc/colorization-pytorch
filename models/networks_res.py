...
class SIGGRAPHGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, use_tanh=True, classification=True):
        ...
        model1 +=[norm_layer(64), nn.ReLU(True),]
        ...
        # Conv2
        model2_prev = [norm_layer(64), nn.ReLU(True),]
        model2 = [nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model2 +=[norm_layer(128), nn.ReLU(True),]
        model2 += [nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model2_res = [nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        # add a subsampling layer operation

        self.model2_prev = nn.Sequential(*model2_prev)
        self.model2 = nn.Sequential(*model2)
        self.model2_res = nn.Sequential(*model2_res)

        # Conv3
        model3_prev = [norm_layer(128), nn.ReLU(True),]
        model3 = [nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model3 += [norm_layer(256), nn.ReLU(True),]
        model3 += [nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model3 += [norm_layer(256), nn.ReLU(True),]
        model3 += [nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model3_res = [nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        # add a subsampling layer operation

        self.model3_prev = nn.Sequential(*model3_prev)
        self.model3 = nn.Sequential(*model3)
        self.model3_res = nn.Sequential(*model3_res)

        # Conv4
        # model47=[nn.ReflectionPad2d(1),]
        model4_prev = [norm_layer(256), nn.ReLU(True),]
        model4 = [nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model4 += [norm_layer(512), nn.ReLU(True),]
        model4 += [nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model4 += [norm_layer(512), nn.ReLU(True),]
        model4 += [nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        model4_res = [nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, bias=use_bias),]

        self.model4_prev = nn.Sequential(*model4_prev)
        self.model4 = nn.Sequential(*model4)
        self.model4_res = nn.Sequential(*model4_res)

        # Conv5
        model5 = [norm_layer(512), nn.ReLU(True),]
        ...
        # Conv6
        model6 = [norm_layer(512), nn.ReLU(True),]
        ...
        # Conv7
        model7 = [norm_layer(512), nn.ReLU(True),]
        ...
        model1short10 = [norm_layer(64), nn.ReLU(True),
                         nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=use_bias),]
        # add the two feature maps above

        model10=[norm_layer(128), nn.ReLU(True),]
    ...
    def forward(self, input_A, input_B, mask_B):
        conv1 = self.model1(torch.cat((input_A,input_B,mask_B),dim=1))

        conv2_prv = self.model2_prev(conv1[:,:,::2,::2])
        conv2_cur = self.model2(conv2_prv)
        conv2_res = self.model2_res(conv2_prv)
        conv2 = conv2_cur + conv2_res

        conv3_prv = self.model3_prev(conv2[:,:,::2,::2])
        conv3_cur = self.model3(conv3_prv)
        conv3_res = self.model3_res(conv3_prv)
        conv3 = conv3_cur + conv3_res

        conv4_prv = self.model4_prev(conv3[:,:,::2,::2])
        conv4_cur = self.model4(conv4_prv)
        conv4_res = self.model4_res(conv4_prv)
        conv4 = conv4_cur + conv4_res

        conv5_cur = self.model5(conv4)
        conv5 = conv4 + conv5_cur

        conv6_cur = self.model6(conv5)
        conv6 = conv5 + conv6_cur

        conv7_cur = self.model7(conv6)
        conv7 = conv6 + conv7_cur

        conv8_up = self.model8up(conv7) + self.model3short8(conv3)

        conv8_cur = self.model8(conv8_up)
        conv8 = conv8_cur + conv8_up
        ...
        if(self.classification):
            ...
            conv9_cur = self.model9(conv9_up)
            conv9 = conv9_cur + conv9_up
